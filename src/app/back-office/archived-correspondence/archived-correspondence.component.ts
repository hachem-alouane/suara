...
@Component({
  selector: 'app-archived-correspondence',
  standalone: true,
  imports: [
    TableComponent,
    TranslateModule,
  ...
  templateUrl: './archived-correspondence.component.html',
  styleUrl: './archived-correspondence.component.scss',
  providers: [DialogService, DialogUtilsService],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ArchivedCorrespondenceComponent implements AfterViewInit, OnInit {
  ...
  columns: ICols[] = [
    {
      ...
      type: TableType.ARCHIVED_CORRESPONDENCE,
    },
  ...
  ngAfterViewInit(): void {
    this.table.tableService.initTable(
      TableType.ARCHIVED_CORRESPONDENCE,
      this.myForm
    );
  ...
  private listenToCurrentUser() {
    this.userAuthService.currentUser
      ...
      .subscribe((user) => {
        const canView = hasModulePermission('ARCHIVED_CORRESPONDENCE', 'VIEW');
        const canTransfer = hasModulePermission(
          'ARCHIVED_CORRESPONDENCE',
          'DISTRIBUTE'
        );

        if (canView(user?.modules ?? [])) this.actionsTable.push('view');
        if (canTransfer(user?.modules ?? []))
          this.actionsTable.push('distribution');
      });
  }
  ...
  goToView(data: any) {
    this.router.navigateByUrl('/courrier-original/' + data?.data?.id);
  }
  showDistribution(event: { data: any }) {
    const { data } = event;
    this.dialogUtilsService
      .openDialog(
        DispatchComponent,
        this.localeService.translate('SEND_EMAIL'),
        {
          id: data?.id,
        },
        false,
    ...
    this.ref.onClose.pipe(take(1)).subscribe((result) => {
      if (result) {
        this.dialogUtilsService.showSuccessMessage(
          this.localeService.translate('EMAIL_SENT_SUCCESS')
        );
        this.table.tableService.setDataList();
      }
    });
